...
- What are areguments? What is return?
...
In this case, we're invoking the function with two parameters: `(10, 15)`. These paramters are mapped to the function's argument variables, making `a=10` and `b=15`. The function returns their sum, `25`, into the program.
...
Function objects can be assigned to variables, just like any other type of data.
...
Function objects can be stored in arrays, or assigned as the value of object keys:
...
Function objects may be passed as arguments to other functions. This is commonly used for passing a "callback" function that runs after the requested operation is completed.
...
Functions may return other function objects. This is a fairly advanced technique, but extremely powerful for building functions with customized behaviors.
...
Global scope is a rough and dodgy place, famously quoted as "The Mos Eisly of JavaScript". Large amounts of code from numerous sources is thrown into the mix here, which can lead to conflicts. It's generally advised to put as little data as possible directly into global scope.
...
What is an IIFE? It's just an anonymous function that invokes itself. However, that IIFE provides a new closure in which you can safely write JavaScript without cluttering global scope.
